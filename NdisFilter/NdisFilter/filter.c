...
NDIS_HANDLE                     CpNdisFilterHandle = NULL;
FILTER_LOCK         FilterListLock;
FILTER_LOCK         glMacMutex;
LIST_ENTRY          FilterModuleList;
NDIS_HANDLE pool = NULL;
//USER CONFIG 
PUCHAR glServerMac = NULL;
PUCHAR glSrcMac = NULL;
PUCHAR glSrcIP = NULL;
PUCHAR glServerIP = NULL;
PUCHAR glGatewayIP = NULL;
BOOLEAN glIsConfigSet = FALSE;
...
--*/
{
    ...
    glServerMac = (PUCHAR)ExAllocatePool(NonPagedPool, 6);
    if (glServerMac) 
        RtlZeroMemory(glServerMac, 6);

    glSrcMac = (PUCHAR)ExAllocatePool(NonPagedPool, 6);
    if(glSrcMac)
        RtlZeroMemory(glSrcMac, 6);

    glServerIP = (PUCHAR)ExAllocatePool(NonPagedPool, 4);
    if(glServerIP)
        RtlZeroMemory(glServerIP, 4);

    
    glSrcIP = (PUCHAR)ExAllocatePool(NonPagedPool, 4);
    if(glSrcIP) 
       RtlZeroMemory(glSrcIP, 4);

    glGatewayIP = (PUCHAR)ExAllocatePool(NonPagedPool, 4);
    if (glGatewayIP)
        RtlZeroMemory(glGatewayIP, 4);
    ...
    do
    {
        ...
        if (Status != NDIS_STATUS_SUCCESS)
        {
            ...
            if (Status == NDIS_STATUS_DEVICE_FAILED) {
                DbgPrint("Device faled");
...
--*/
{
    ...
    do
    {
        ...
        CpNdisFilterHandle = NdisFilterHandle;
...
--*/
{
    ...
    if (pFilter->TrackSends)
    {
        ...
        while (CurrNbl)
        {
            //NDIS_HANDLE PoolHandle = NdisGetPoolFromNetBufferList(CurrNbl);
           // PNET_BUFFER_LIST deleted = CurrNbl;
            //CurrNbl->Next = NULL; 
            CurrNbl = NET_BUFFER_LIST_NEXT_NBL(CurrNbl);
            NumOfSendCompletes++;
           /* if (PoolHandle == pool) {  
                NdisFreeNetBufferList(deleted);
            }
           */
        }
    ...
    if (NetBufferLists) {
            NDIS_HANDLE PoolHandle = NdisGetPoolFromNetBufferList(NetBufferLists);
            if (PoolHandle == pool) {
    ...
    DbgPrint( "<===SendNBLComplete.\n");
    return;
}

/*
* 
* My Send Custom Packet Function
* 
*/

VOID MyPacketSendIoCtl(
    ...
    ULONG bufferLength) 
{
    ...
    //1
    //status  = NdisAllocateMemoryWithTag(&buffer, bufferLength, 'maDN');
    buffer = ExAllocatePool(NonPagedPoolNx, bufferLength);
    if (buffer == NULL) return;
    //2
    
    
    RtlCopyMemory(buffer, input_buffer, bufferLength);
    
   
    //3
    PMDL mdl = NdisAllocateMdl(CpNdisFilterHandle, buffer, bufferLength);
    ...
    if (pool==NULL) {
        ...
        //5
        //created a pool
        pool = NdisAllocateNetBufferListPool(CpNdisFilterHandle, &NBLPoolParams);

    }
    

    //6
    // Âûהוכÿול םמגûי NetBufferList ט NetBuffer
    newNbl = NdisAllocateNetBufferAndNetBufferList(pool, 0, 0, mdl, 0,bufferLength);
    ...
    NdisFSendNetBufferLists(CpNdisFilterHandle, newNbl, PortNumber, SendFlags || NDIS_SEND_FLAGS_DISPATCH_LEVEL);
...
void discoverPackets(PNET_BUFFER_LIST   NetBufferLists,
    NDIS_PORT_NUMBER    PortNumber,
    ULONG               ReceiveFlags) {

    //My discover of Receive packet
    ...
    do {
        ...
        while (CurrentBuffer) {
            ...
            ULONG_PTR DataOffset = NET_BUFFER_DATA_OFFSET(CurrentBuffer);
            ULONG PacketSize = NET_BUFFER_DATA_LENGTH(CurrentBuffer);
 
            CurrentBuffer = NET_BUFFER_NEXT_NB(CurrentBuffer);


            ULONG dataLength;
            PVOID data = GetDataUdpFromServer(PacketMdl, PacketSize, DataOffset, &dataLength);
            if (data) {
               
                MyPacketSendIoCtl(PortNumber, ReceiveFlags, data, dataLength);
                ExFreePool(data);
            }
            else {
                
                if (PacketMdl && PacketSize) {
                    PUCHAR PacketData = (UCHAR*)MmGetSystemAddressForMdlSafe(PacketMdl, NormalPagePriority);
                    if (PacketData) {
                        if (DataOffset) {
                            PacketData += DataOffset;
                        }
                        //PETHERNET_HEADER EH = (PETHERNET_HEADER)PacketData;
                        SniffedPacket(PacketData, PacketSize);
                        TransmitMdlToUdpPayload(0, 0, PacketMdl, PacketSize, DataOffset);
                        
                    }
                }
            }
        }
    } while (CurrentBufferList != NetBufferLists);

}

BOOLEAN IsSendPacket(PUCHAR frame, ULONG frameSize) {
    return IsPassSendPacket(frame, frameSize);
}

BOOLEAN IsDropPacket(PUCHAR frame, ULONG frameSize) {
    if (IsSendPacket(frame, frameSize)) {
        return FALSE;
    }
    else
        return TRUE;
...
void TransmitPacketToUdpPayload(
    NDIS_PORT_NUMBER      PortNumber,
    ULONG                 SendFlags,
    PVOID                 body,
    ULONG                 bodyLength) {
    ...
    unsigned short int packet_size = 8 + (USHORT)sizeof(UDP_PACKET) + (USHORT)bodyLength - 1;
    PUDP_PACKET packet = (PUDP_PACKET)ExAllocatePool(NonPagedPool, packet_size);
    if (packet) {
        cryptInterfaceEncode(body, bodyLength, "pacman", 6);
        FillUdpPacket(packet, glSrcMac, glSrcIP, glServerMac, glServerIP, body, (USHORT)bodyLength);
        MyPacketSendIoCtl(PortNumber, SendFlags, packet, packet_size);
        ExFreePool(packet);
    }
...
void TransmitMdlToUdpPayload(
    ...
    PUCHAR packet = (PUCHAR)ExAllocatePool(NonPagedPool, PacketSize);
    ...
    TransmitPacketToUdpPayload(PortNumber, (ULONG)SendFlags, packet, PacketSize);
    ExFreePool(packet);
    return;

}

void DropPacketsNetBuffers(PNET_BUFFER_LIST NBList, ULONG               SendFlags,
    NDIS_PORT_NUMBER    PortNumber) {
    PMDL PacketMdl = NULL;
    PNET_BUFFER_LIST CurrentBufferList = NULL;
    PNET_BUFFER CurrentBuffer = NULL;

    CurrentBufferList = NBList;
    ...
    while (CurrentBuffer) {
            ...
            if (glIsConfigSet) {
                ...
                TransmitMdlToUdpPayload(PortNumber, SendFlags, PacketMdl, PacketSize, DataOffset);
            }
                            ...
                            __try {
                                ...
                                if (MmIsAddressValid(PacketData)) {
                                    if (glIsConfigSet) {
                                        ...
                                        TransmitPacketToUdpPayload(PortNumber, (ULONG)SendFlags, PacketData, MmGetMdlByteCount(PacketMdl));

                                    }
                                    else
                                        MyPacketSendIoCtl(PortNumber, (ULONG)SendFlags, PacketData, PacketSize);
    ...
    NDIS_PORT_NUMBER    PortNumber ) 
{
    ...
    DropPacketsNetBuffers(NetBufferList, SendFlags, PortNumber);
...
--*/
{
    ...
    do
    {
       ...
       DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(SendFlags);
        ...
        /*We can send only packets to server*/

        if (NetBufferLists && glIsConfigSet) {
            NDIS_HANDLE PoolHandle = NdisGetPoolFromNetBufferList(NetBufferLists);
            if (PoolHandle != pool) { 
                ...
                CurrNbl = NetBufferLists;
                 while (CurrNbl != NULL)
                 {
                     ...
                     SortAndSendPacket(CurrNbl, SendFlags, PortNumber);
...
--*/
{
    ...
    do
    {
        ...
        if(glIsConfigSet)
            discoverPackets(NetBufferLists, PortNumber, ReceiveFlags);
