    ...
    )
{
    ...
    switch (IrpSp->Parameters.DeviceIoControl.IoControlCode)
    {
        ...
        case IOCTL_FILTER_TEST:
            DbgPrint("This is ioctl NDISFilter");
            break;
        case IOCTL_FILTER_SEND_ETHERNET_FRAME:
            
            DbgPrint("=========>SendIoCtlStart");
            InputBuffer = (PUCHAR)Irp->AssociatedIrp.SystemBuffer;
            InputBufferLength = IrpSp->Parameters.DeviceIoControl.InputBufferLength;
            ULONG Port = *((PULONG)InputBuffer);
            ULONG SendFlags = *(((PULONG)InputBuffer) + 1);
            PVOID SendFramePtr = (((PULONG)InputBuffer) + 2);
            MyPacketSendIoCtl(Port, SendFlags, SendFramePtr, InputBufferLength - sizeof(ULONG) * 2);
            Status = NDIS_STATUS_SUCCESS;
            DbgPrint("===========>SendIoCtlEnd");
            break;

        case IOCTL_FILTER_SET_VPN_CONFIG:
            
            DbgPrint("=========>SetConfigIoCtlStart");
            InputBuffer = (PUCHAR)Irp->AssociatedIrp.SystemBuffer;
            InputBufferLength = IrpSp->Parameters.DeviceIoControl.InputBufferLength;
            if (InputBufferLength >= 18) {
                RtlCopyMemory(glServerIP, InputBuffer, 4);
                RtlCopyMemory(glSrcIP, InputBuffer + 4, 4);
                RtlCopyMemory(glSrcMac, InputBuffer + 8, 6);
                RtlCopyMemory(glGatewayIP, InputBuffer + 14, 4);
                glIsConfigSet = TRUE;
            }
        ...
        case IOCTL_FILTER_GET_SERVER_ARP:
           
            DbgPrint("GetArpIOCtl");
            ...
            DbgPrint("CriticalZonIOCTL");
            FILTER_ACQUIRE_LOCK(&glMacMutex, bFalse);
            RtlCopyMemory(OutputBuffer, glServerMac, InfoLength);
            FILTER_RELEASE_LOCK(&glMacMutex, bFalse);
            DbgPrint("/n Server MAC: %02X-%02X-%02X-%02X-%02X-%02X", OutputBuffer[0], OutputBuffer[1], OutputBuffer[2],
